...
namespace MyWeb.Controllers
{
    [Route("nhaxuatban")]
    [ApiController]
    public class NhaXuatBanController : ApiCoreController
    {
        private readonly INhaXuatBanService _service;

        public NhaXuatBanController(INhaXuatBanService service)
        {
            _service = service;
        }
        ...
        public async Task<ApiResponse> Add([FromBody] NhaXuatBan item)
        {
            try
            {
        ...
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [Consumes("application/json")]
        public async Task<ApiResponse> Updated(NhaXuatBan item)
        {
            try
            {
                await Task.Delay(0);
