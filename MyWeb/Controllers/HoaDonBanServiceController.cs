...
namespace MyWeb.Controllers
{
    [Route("hoadonban")]
    [ApiController]
    public class HoaDonBanServiceController : ApiCoreController
    {
        private readonly IHoaDonBanService _service;

        public HoaDonBanServiceController(IHoaDonBanService service)
        {
            _service = service;
        }
        ...
        public async Task<ApiResponse> Add([FromBody] HoaDonBan item)
        {
            try
            {
        ...
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [Consumes("application/json")]
        public async Task<ApiResponse> Updated(HoaDonBan item)
        {
            try
            {
                await Task.Delay(0);
