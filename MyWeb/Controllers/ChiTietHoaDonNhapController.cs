...
{
    [Route("chitiethoadonnhap")]
    [ApiController]
    public class ChiTietHoaDonNhapController : ApiCoreController
    {
        private readonly IChiTietHoaDonNhapService _service;

        public ChiTietHoaDonNhapController(IChiTietHoaDonNhapService service)
        {
            _service = service;
        }
        ...
        public async Task<ApiResponse> Add([FromBody] ChiTietHoaDonNhap item)
        {
            try
            {
        ...
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [Consumes("application/json")]
        public async Task<ApiResponse> Updated(ChiTietHoaDonNhap item)
        {
            try
            {
                await Task.Delay(0);
